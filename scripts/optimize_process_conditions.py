...
def main():
    ...
    missing = [c for c in args.feature_cols + args.targets if c not in df.columns]
    if missing:
        raise ValueError(f'Columns missing from {args.stage1_csv}: {missing}')
    df = df.dropna(subset=args.feature_cols + args.targets)

    model = ProcessCorrelationModel(model=args.model).fit(df, args.feature_cols, args.targets)
    cv_r2 = model.cross_val_r2()
    print('Cross-validated R^2 per target (sanity-check before trusting the search below):')
    for t, r2 in cv_r2.items():
        print(f'  {t}: {r2:.3f}')
        if r2 < 0.3:
            print(f'    WARNING: low R^2 for {t} -- the surrogate search below may not be reliable yet; '
                  f'collect more Stage 1 devices or narrow the process window.')

    params = [FitParam(name=col, type='range', value=float(df[col].mean()), bounds=list(bounds[col]),
                        display_name=col) for col in args.feature_cols]

    agent = MLSurrogateAgent(params, model, targets=args.targets, minimize=minimize)

    num_free = len(params)
    optimizer = axBOtorchOptimizer(params=params, agents=agent, models=['SOBOL', 'BOTORCH_MODULAR'],
                                    n_batches=[max(10, 2 * num_free), 30], batch_size=[1, 1])
    optimizer.optimize()

    data = optimizer.ax_client.summarize()
    sort_col = optimizer.all_metrics[0]
    ascending = agent.minimize[0]
    top = data.sort_values(sort_col, ascending=ascending).head(args.n_suggestions)
    top = top[args.feature_cols + optimizer.all_metrics]
