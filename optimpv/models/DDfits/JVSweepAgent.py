                    ...
                    if type(ret) == int:
                        ...
                        if not all([(r == 0 or r == 95) for r in ret]):
                            ...
                            for i in range(len(ret)):
                                print(mess[i])
                            return np.nan
                # # run the simulations for each Gfrac in parallel using joblib
                # results = Parallel(n_jobs=max_jobs)(delayed(self.run_1_Gfrac_JV)(self.scan_speed, self.direction, Gfrac, self.Vmin, self.Vmax, self.steps, UUID, stabilized=self.stabilized, cmd_pars=clean_pars) for Gfrac in Gfracs_eff)
                # for res in results:
                #     ret, mess, rms = res
                #     if type(ret) == int:
                #         if not (ret == 0  or ret == 95) :
