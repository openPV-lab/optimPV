...
def onehot2ordinal(x, categorical_dims):
    ...
    categorical_dims : list[list[int]]
        One-hot column groups for each categorical variable.

    Returns
    -------
    torch.Tensor
        Ordinal representation of the categorical data.
    """
...
def ordinal2onehot(x, n_categories):
    ...
    -------
    torch.Tensor
        One-hot encoded categorical vector.
    """
...
class GP(ExactGP):
    ...
    def __init__(self, train_x, train_y, kern, likelihood,
        ...
        train_y : torch.Tensor
            Training targets.
        kern : gpytorch.kernels.Kernel
            Base covariance kernel.
        likelihood : gpytorch.likelihoods.Likelihood
            Likelihood module.
        outputscale_constraint : gpytorch.constraints.Interval
            Constraint applied to the output scale.
        ard_dims : int | None
            Number of ARD dimensions.
        cat_dims : list[int] | None, optional
            Categorical dimension indices.
        """
    ...
    def forward(self, x):
        """Evaluate the GP prior or posterior at the provided inputs.
        ...
        gpytorch.distributions.MultivariateNormal
            Multivariate normal distribution over the outputs.
        """
...
def train_gp(train_x, train_y, use_ard, num_steps, kern='transformed_overlap', hypers={},
    ...
    ----------
    train_x : torch.Tensor
        Training inputs in normalized coordinates.
    train_y : torch.Tensor
        Standardized training targets.
    use_ard : bool
        Whether ARD is enabled for the GP kernel.
    num_steps : int
        Number of optimization steps for GP hyperparameters.
    kern : str, optional
        Kernel identifier.
    hypers : dict, optional
        Optional initial hyperparameters or state dictionary entries.
    cat_dims : list[int] | None, optional
        Categorical dimension indices for mixed kernels.
    cont_dims : list[int] | None, optional
        Continuous dimension indices for mixed kernels.
    int_constrained_dims : list[int] | None, optional
        Continuous dimensions constrained to integer values.
    noise_variance : float | None, optional
        Fixed observation noise variance.
    cat_configs : array-like | None, optional
        Category counts for ordinal kernels.
    **params
...
def local_search(x_center, f: Callable,
    ...
    ----------
    x_center : array-like
        The center of the trust region. In this case, this should be the optimum encountered so far.
    f : Callable
        The function handle to evaluate x on (the acquisition function, in this case)
    config : array-like
        Configuration of the categorical variables.
    max_hamming_dist : int | float
        Maximum Hamming distance from x_center
    n_restart : int, optional
        Number of restarts
    batch_size : int, optional
        Number of returned points.
    step : int, optional
        Number of maximum local search steps the algorithm is allowed to take.

    Returns
    -------
    tuple[numpy.ndarray, numpy.ndarray]
        Selected points and their acquisition values.
    """
...
def interleaved_search(x_center, f: Callable,
    ...
    f : Callable
        Function handle (normally this should be the acquisition function)
    cat_dims : list[int]
        The indices of the categorical dimensions
    cont_dims : list[int]
        The indices of the continuous dimensions
    config : array-like
        The config for the categorical variables
    ub : array-like
        Upper bounds (trust region boundary) for the continuous variables
    lb : array-like
        Lower bounds (trust region boundary) for the continuous variables
    max_hamming_dist : int | float
        Maximum hamming distance boundary (for the categorical variables)
    n_restart : int, optional
        Number of restarts of the optimisaiton
    batch_size : int, optional
        Number of returned points.
    interval : int, optional
        Number of steps to switch over (to start with, we optimise with n_interval steps on the continuous
        variables via a first-order optimiser, then we switch to categorical variables (with the continuous ones fixed)
        and etc.
    step : int, optional
        Maximum number of search allowed.
