...
from .localbo_utils import from_unit_cube, latin_hypercube, to_unit_cube, onehot2ordinal
import torch
...
class MixedOptimizer(Optimizer):
    ...
    def __init__(self, config,
        ...
        ----------
        config : list
            e.g. [2, 3, 4, 5] -- denotes there are 4 categorical variables, with numbers of categories
            being 2, 3, 4, and 5 respectively.
        lb : numpy.ndarray
            Lower bounds of the mixed search space.
        ub : numpy.ndarray
            Upper bounds of the mixed search space.
        cont_dims : list[int]
        ...
        n_init : int | None, optional
            Number of initialization points to use, will be ignored if initial seeding is provided.
        wrap_discrete : bool, optional
            Whether one-hot discrete samples should be projected back to a
            valid one-hot encoding.
        guided_restart : bool, optional
        ...
        **kwargs
            Additional options forwarded to the mixed CASMOPOLITAN optimizer.
        """
    ...
    def restart(self):
        """Reset or re-seed the mixed local optimizer after TR collapse."""
        ...
        if self.guided_restart and len(self.casmopolitan._fX):
            ...
            self.auxiliary_gp = train_gp(
                ...
                fX_tr_torch,
                False,
                self.casmopolitan.n_training_steps,
                cat_dims=self.d_cat,
                cont_dims=self.d_cont,
                kern='mixed',
                noise_variance=self.kwargs[
                    'noise_variance'
                ] if 'noise_variance' in self.kwargs else None,
            )
    ...
    def suggest(self, n_suggestions=1):
        """Generate the next batch of mixed-space suggested points.
        ...
        -------
        numpy.ndarray
            Suggested points in mixed ordinal/continuous representation.
        """
        ...
        if n_adapt > 0:
            if len(self.casmopolitan._X) > 0:  # Use random points if we can't fit a GP
                ...
                next = self.casmopolitan._create_and_select_candidates(
                    ...
                    n_training_steps=self.casmopolitan.n_training_steps,
                    hypers={},
                )[-n_adapt:, :]
