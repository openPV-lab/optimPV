...
class MixtureKernel(Kernel):
    ...
    def __init__(self, categorical_dims,
                 ...
                 continuous_ard=True,
                 **kwargs):
        """Initialize the mixed categorical and continuous kernel.

        Parameters
        ----------
        categorical_dims : list[int]
            The dimension indices that are categorical/discrete
        continuous_dims : list[int]
            The dimension indices that are continuous
        integer_dims : list[int] | None, optional
            The **continuous indices** that additionally require integer constraint.
        lamda : float | None, optional
            \in [0, 1]. The trade-off between product and additive kernels. If this argument is not supplied, then
            lambda will be optimised as if it is an additional kernel hyperparameter
        categorical_kern_type : str, optional
            'overlap', 'type2'
        continuous_kern_type : str, optional
            'rbf' or 'mat52' (Matern 5/2)
        categorical_lengthscale_constraint : gpytorch.constraints.Constraint | None, optional
            If supplied, the constraint on the lengthscale of the categorical kernel
        continuous_lengthscale_constraint : gpytorch.constraints.Constraint | None, optional
            If supplied, the constraint on the lengthscale of the continuous kernel
        categorical_ard : bool, optional
            Whether to use Automatic Relevance Determination (ARD) for categorical dimensions
        continuous_ard : bool, optional
            Whether to use ARD for continuous dimensions
        **kwargs
    ...
    def forward(self, x1, x2, diag=False,
        ...
        ----------
        x1 : torch.Tensor
            Joint input tensor or categorical tensor.
        x2 : torch.Tensor
            Joint input tensor or categorical tensor.
        diag : bool, optional
            Whether to return only the diagonal.
        x1_cont : torch.Tensor | None, optional
            Continuous part of ``x1`` when categorical and continuous inputs are
            supplied separately.
        x2_cont : torch.Tensor | None, optional
            Continuous part of ``x2`` when categorical and continuous inputs are
            supplied separately.
        **params
            Additional kernel evaluation parameters.

        Returns
        -------
        torch.Tensor
            Kernel matrix or diagonal kernel values.
        """
...
class WrappedRBF(RBFKernel, WrappedMatern):
    ...
    def __init__(self, integer_dims=None, **kwargs):
        """Initialize the wrapped RBF kernel."""
        super(WrappedRBF, self).__init__(**kwargs)
        self.integer_dims = integer_dims

    def forward(self, x1, x2, diag=False, **params):
        """Evaluate the wrapped RBF kernel."""
        x1, x2 = wrap(x1, x2, self.integer_dims)
        return super().forward(x1, x2, diag=diag, **params)
...
class CategoricalOverlap(Kernel):
    ...
    def forward(self, x1, x2, diag=False, last_dim_is_batch=False, **params):
        """Evaluate the categorical overlap kernel."""
...
class TransformedCategorical(CategoricalOverlap):
    ...
    def forward(self, x1, x2, diag=False, last_dim_is_batch=False, exp='rbf', **params):
        """Evaluate the transformed categorical kernel."""
        diff = x1[:, None] - x2[None, :]
        diff[torch.abs(diff) > 1e-5] = 1
        diff1 = torch.logical_not(diff).float()
