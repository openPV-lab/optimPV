            ...
            if isinstance(trial, BatchTrial):
                ...
                for arm in trial.arms:
                    ...
                    df_dict = {
                        ...
                        "arm_name": arm.name,
                        "mean": branin_data.get(name_),#self.name),
...
class axBOtorchOptimizer_runner(axBOtorchOptimizer):
    ...
    def _optimize(self):
        ...
        if len(_obj.keys()) == 1:
            ...
            obj = Objective(metric=BraninForMockJobMetric(name=list(_obj.keys())[0], agents = self.agents, pool = q, tmp_dir = tmp_dir, parallel_agents = parallel_agents), minimize=True)
        else:
